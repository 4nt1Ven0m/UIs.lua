...
function library:AddTab(title, pos)
	...
	function tab:Init()
        ...
        -- Calculate text size.
        local textSize = textService:GetTextSize(self.title, 18, Enum.Font.Code, Vector2.new(9e9, 9e9))
        local sizeX = textSize.X + 20 -- Increased padding

        -- Cool button appearance
        local bgColor = Color3.new(0.2, 0.2, 0.2)  -- Dark background
        local textColor = Color3.new(0.9, 0.9, 0.9) -- Slightly off-white text
        local hoverBgColor = Color3.new(0.3, 0.3, 0.3) -- Darker on hover
        local selectedBgColor = Color3.new(0.1, 0.5, 0.8) -- Highlighted color

        -- Create the button
        self.button = library:Create("TextButton", { -- Changed to TextButton for better hover effect
            Position = UDim2.new(0, library.tabSize, 0, 22),
            Size = UDim2.new(0, sizeX, 0, 30),
            ...
            ClipsDescendants = true,
            Parent = library.main,
            Style = Enum.ButtonStyle.RobloxRound, -- Optional: Apply a Roblox style
        })

        -- Visual Tweaks
        self.button.TextScaled = true  -- Use TextScaled for better text sizing
        self.button.TextStrokeTransparency = 0.5 -- Add a subtle text stroke
        self.button.TextStrokeColor3 = Color3.new(0, 0, 0)

        library.tabSize = library.tabSize + sizeX

        -- Button Events (Hover & Click)

        -- Hover effect
        self.button.MouseEnter:Connect(function()
            if library.selectedTab ~= self then -- Only change if not selected
                self.button.BackgroundColor3 = hoverBgColor
            end
        end)

        self.button.MouseLeave:Connect(function()
            if library.selectedTab ~= self then
                self.button.BackgroundColor3 = bgColor
            end
        end)

        -- Selection logic.  Use the library's selectTab function, which
        -- is assumed to handle all other tab related stuff.
        self.button.MouseButton1Click:Connect(function()
            library:selectTab(self)
        end)


        -- Initialization of columns (if any)
    ...
    -- Check to make sure the tab is initialized if the self.hasInit variable is true.
